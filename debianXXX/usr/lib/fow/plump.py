...
#def get_subdir(path):
    #"""
    #Extracts subdir from the given path within a fw.
    #Returns None, if path is not within a fw.
    #Example: 
...
def checkParams(_actual, _rules):
    ...
    Take care, that the none-path can not have optional options. Put them
    in the end of the list. For instance:
    fow backup [-t] | fow backup -p PATH [-t]
    must be in this order:
    rules = [   [dict(atom=atomPath, obligat=True),
                 dict(atom=atomTest, obligat=False)],
                [dict(atom=atomNone, obligat=True)], #After other paths
                [dict(atom=atomTest, obligat=True)]  #Own rule for -t
             ]
    ...
    for path in _rules:
        ...
        for node in path:
            ...
            or node['atom']['short'] in _actual['shorts']:
                ...
                message = 'Missing mandatory option --' + \
                            str(node['atom']['name'])
                valid = False
                break
...
def readConfig():
    ...
    settings = dict()
    try:
        with open(get_fow_root() + '/' + DIR_FOW + 
                  '/setting.pickle', 'rb') as data:
...
def show_tasks():
    ...
    for each_folder in os.listdir(get_path(DIR_02)):
        ...
        for each_task in os.listdir(get_path(DIR_02) + '/' + each_folder):
            stats = get_status(get_path(DIR_02) + '/' + each_folder + '/' + each_task)
