...
def images_get_exifs(path, file_names):
    """
    Returns a list of dictionaries with all supported exif informations for the give files.
    file_names: List with file names without a path
    path: Path to the files
    Example
        return [
        dict(file='img01.jpg', gps=dict(lan=1.0, lat=49,54.318340N), title='A huge tree',
        ... ]
    """
    ret = []
    for each in file_names:

        cmd = 'exiftool -T -filename -gpslatitude -gpslongitude -title {}/{}'.format(path, each)
        # cmd = 'exiftool -T -filename -gpslatitude -gpslongitude -title {}'.format('/home/chris/PycharmProjects/fowPycharm/fw/01_Import/jpg/img002.jpg /home/chris/PycharmProjects/fowPycharm/fw/01_Import/jpg/20160906-101452-img001.jpg')
        ...
        try:
            ...
            ret.append(dict(name=each, title=None, gps=None))
            # print(str(e))
            continue
        ...
        ret.append(dict(name=values[0], title=title, gps=gps))

    return ret


def images_get_exifsXXX(path, file_names):
    """
    Returns a list of dictionaries with all supported exif informations for the give files.
    file_names: List with file names without a path
    path: Path to the files
    ...
    names = ""
    for each in file_names:
        names = "{} {}/{}".format(names, path, each)

    names = ''
    names = "{}/{}".format(path, file_names[0])

    cmd = 'exiftool -T -filename {}'.format(names)

    cmd = 'exiftool -T -filename -gpslatitude -gpslongitude -title {}'.format('/home/chris/PycharmProjects/fowPycharm/fow/01_Import/jpg/img002.jpg /home/chris/PycharmProjects/fowPycharm/fow/01_Import/jpg/20160906-101452-img001.jpg')
    print('images_get_exifs() cmd={}'.format(cmd))
    try:
        b = subprocess.check_output(
            cmd,
        ...
        print('ERROR exiftool failed for {}'.format(cmd))
        print(str(e))
        return None

    values = str(b)
    print('images_get_exifs() value={}'.format(str(values)))

    value_list = values.split('\n')

    for each in value_list:
        values = each.split('\t')
        ret.pop(dict(name=values[0], title=values[3], gps=dict(lat=values[1], lon=values[2])))
...
def image_get_location(filename):
    ...
    lat_value = image_get_exiftool_tag(filename, 'gpslatitude')
    ...
    lon_value = image_get_exiftool_tag(filename, 'gpslongitude')
...
def image_get_exiftool_tag(filename, tagname):
    """
    Reads the tag value of the given file and returns the value as string.
    Returns None, if not found.
    For example:
        image_get_gps_tag('img01.jpg', 'gpslatitude')
        return '50 deg 10' 37.92" N'
    """
    cmd = 'exiftool -T -{} {}'.format(tagname, filename)
    try:
        b = subprocess.check_output(
            cmd,
            stderr=subprocess.STDOUT,
...
def deprecated_image_get_gps_tag(filename, tagname):
    """
